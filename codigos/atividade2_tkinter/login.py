...
class LoginApp(tk.Tk):
    def __init__(self):
        ...
        frame = tk.Frame(self, bg="#FFE4E1", padx=20, pady=20, relief="raised", borderwidth=2)
        frame.place(relx=0.5, rely=0.5, anchor="center")

        try:
            imagem = Image.open("sistema2_tkinter/img/Livro.png")
        ...
        self.login_botao = tk.Button(frame, text="Login", bg="#FFB6C1", fg="white", command=self.login)
        self.login_botao.pack(pady=5)

        self.registrar_botao = tk.Button(frame, text="Cadastrar", bg="#FFB6C1", fg="white", command=self.abrir_tela_cadastro)
    ...
    def login(self):
        ...
        if verificar_usuario(usuario, senha):  # type: ignore
            messagebox.showinfo("Login", 'Login Realizado com Sucesso!')
            self.destroy()#fecha janela de login
            DashboardApp()#abre a janela
        else:
